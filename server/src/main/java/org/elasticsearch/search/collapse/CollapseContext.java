...
public class CollapseContext {
    ...
    public CollapseContext(String fieldName,
                           MappedFieldType fieldType,
                           List<InnerHitBuilder> innerHits, String countryStr, int sortFieldSize) {
        this.fieldName = fieldName;
        this.fieldType = fieldType;
        this.innerHits = innerHits;
        if (countryStr != null && countryStr.equals("") == false) {
            firstDataList = Arrays.asList(countryStr.split(","));
        }
    ...
    public CollapsingTopDocsCollector<?> createTopDocs(Sort sort, int topN, boolean trackMaxScore) {
        if (fieldType instanceof KeywordFieldMapper.KeywordFieldType) {
            return CollapsingTopDocsCollector.createKeyword(fieldType.name(), sort, topN, trackMaxScore, firstDataList, sortFieldSize);
        } else if (fieldType instanceof NumberFieldMapper.NumberFieldType) {
            return CollapsingTopDocsCollector.createNumeric(fieldType.name(), sort, topN, trackMaxScore, firstDataList, sortFieldSize);
        } else {
